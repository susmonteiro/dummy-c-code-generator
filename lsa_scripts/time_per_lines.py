...
s_values = [1000, 5000, 10000, 20000, 30000, 40000, 50000, 60000, 70000, 80000, 90000, 100000]  # List of -s values
percentage_of_vars = 0.60
...
for s in s_values:
    ...
    for i in range(1, iterations + 1):
        ...
        filename = f"generated_{s}_t{i}"
        file = filename + ".cpp"
        outfile = filename + ".o"
        warnsfile = "warns.txt"

        if args.generate:
            command = f"python {generator} -s {s} -n {file} -v {percentage_of_vars} > /dev/null"
        file_path = dir + file
        ...
        print(f"File: {file}\tLines: {s}\tProgress: {row - start_row + 1}/{iterations}\t {elapsed_time1:.2f}s")
    ...
    data_sheet.cell(avg_row, 12, value=s)
...
scatter_chart.x_axis.title = 'Number of Lines'
...
compiler_series.marker.symbol = "circle"
