...
for s in s_values:
    ...
    for i in range(1, iterations + 1):
        ...
        if args.generate:
            command = f"python {generator} -s {size_of_file} -n {file} -f -i {s} > /dev/null"
        file_path = dir + file
...
scatter_chart.x_axis.title = 'Percentage of function calls (%)'
