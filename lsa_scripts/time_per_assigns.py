...
s_values = [0.10, 0.20, 0.30, 0.40, 0.5, 0.60, 0.70, 0.80, 0.90]  # List of -s values
...
for s in s_values:
    ...
    for i in range(1, iterations + 1):
        ...
        # command = f"python lifetime_tests_generator.py -s {s} -n {file} > /dev/null"
        if args.generate:
            command = f"python {generator} -s {size_of_file} -n {file} -a -i {s} > /dev/null"
        file_path = dir + file
...
scatter_chart.x_axis.title = 'Percentage of assignments (%)'
